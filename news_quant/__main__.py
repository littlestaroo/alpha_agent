...
from news_quant.agent_pipeline import AgentPipelineConfig, run_agent_pipeline
...
def run_agent_pipeline_main() -> None:
    parser = argparse.ArgumentParser(
        prog="python -m news_quant run-agent-pipeline",
        description="运行第四章多智能体工作流并生成系统报告",
    )
    ...
    parser.add_argument("--max-body-chars", type=int, default=DEFAULT_BODY_CHARS, help="LLM 输入正文最大字符数")
    parser.add_argument("--price-cache", type=Path, default=None, help="价格缓存 CSV")
    args = parser.parse_args()

    summary = run_agent_pipeline(
        AgentPipelineConfig(
            news_path=args.news,
            universe_path=args.universe,
    ...
    print(f"系统报告: {args.out_dir / 'agent_workflow_report.md'}")
    print(f"交易日数: {summary['ranking_summary']['date_count']}")
...
if __name__ == "__main__":
    ...
    elif len(sys.argv) >= 2 and sys.argv[1] == "run-agent-pipeline":
        sys.argv = [sys.argv[0]] + sys.argv[2:]
        run_agent_pipeline_main()
    else:
        run_main()
