#!/usr/bin/env python3
"""Compare ranking performance across different top-k settings."""
...
DEFAULT_OUT_DIR = ROOT / "output" / "chapter3_topk_sensitivity"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="比较不同 top-k 设置下的排序表现")
    ...
    for preset in args.presets:
        ...
        for top_k in args.topk_values:
            ranked_df = build_rankings(profiles_df, top_k=top_k, factor_specs=factor_specs)
            panel_df = attach_future_returns(ranked_df, prices_df)
            daily_df = evaluate_topk(panel_df)
            summary = summarize_performance(daily_df)
            all_summaries[preset][str(top_k)] = {
                "top_k": top_k,
                "factor_weights": [spec.to_dict() for spec in factor_specs],
                **summary,
            }
            summary_rows.extend(flatten_summary(preset, top_k, summary))
            daily_df.to_csv(args.output_dir / f"{preset}_top{top_k}_performance.csv", index=False)

    comparison_df = pd.DataFrame(summary_rows).sort_values(["model", "top_k", "horizon"]).reset_index(drop=True)
    comparison_df.to_csv(args.output_dir / "topk_sensitivity.csv", index=False)
    (args.output_dir / "topk_sensitivity.json").write_text(
